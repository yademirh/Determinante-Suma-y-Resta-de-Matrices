...
class Frame_determinante:
    ...
    @staticmethod
    def create_frame():
        Frame_determinante.frame_def_determinante = CTkFrame(Frame_determinante.root, border_width=3, fg_color="LightBlue4", corner_radius=10)
        ...
        bt = CTkButton(Frame_determinante.frame_def_determinante, text="Back", width=40, command=Frame_determinante.change_destroy)
        bt.place(x=10, y=10)
        ...
        label_defineSize = CTkLabel(container, text="Define el tamaño de la matriz", corner_radius=5, fg_color="LightCyan3", text_color="black", font=("Arial", 14))
        label_defineSize.grid(row=0, column=0)

        options_sizeDet = CTkComboBox(container, values=["1x1","2x2","3x3","4x4","5x5","6x6","7x7"], button_color="LightCyan3", dropdown_hover_color="LightCyan4", command=Frame_determinante.splitChoice , font=("Arial", 14))
        ...
        btn_calculate = CTkButton(frame_left, text="CALCULAR", command=Solution_determinante.show_frame_sol_det, font=("Arial", 16))
        ...
        label_aclaraciones = CTkLabel(frame_right, wraplength=200, text="\tNota:\nEl determinante solo está definido para matrices cuadradas", font=('Arial', 16), fg_color="CadetBlue4", justify="left", corner_radius=5)
        label_aclaraciones.pack(ipady=10, ipadx=20, pady=30)

        label_instrucciones = CTkLabel(frame_right, text="\tInstrucciones:\n* Permite solo el uso de punto (.) para números decimales.\n* Permite slash (/) para indicar una fraccion.\n* No permite cualquier otro tipo de caracteres ni espacio en blanco.\n* Llenar todos los campos", wraplength=200, font=('Arial', 16), fg_color="CadetBlue4", justify="left", corner_radius=5)
        ...
        Solution_determinante.define_root(Frame_determinante.root,Frame_determinante.frame_def_determinante)
